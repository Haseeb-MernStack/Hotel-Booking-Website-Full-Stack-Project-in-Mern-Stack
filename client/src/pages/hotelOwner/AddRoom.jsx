    ...
    <form className=''>
      ...
      <div className='grid grid-cols-2 sm:flex gap-4 my-2 flex-wrap'>
        {Object.keys(images).map((key) => (
          <label htmlFor={`roomImage${key}`} key={key}>
            ...
            <input type="file" className='' accept='image/*' id={`roomImage${key}`} hidden onChange={e => setImages({ ...images, [key]: e.target.files[0] })} />
          </label>
        ))}
      </div>
      ...
      <div className='w-full flex max-sm:flex-col sm:gap-4 mt-4'>
        <div className='flex-1 max-w-48'>
          ...
          <select value={inputs.roomType} onChange={e => setInputs({ ...inputs, roomType: e.target.value })} className='border text-gray-900 opacity-70 border-gray-300 mt-1 rounded p-2 w-full'>
        ...
        <div className=''>
          ...
          <input type="number" placeholder='0' className='border text-gray-900 border-gray-300 mt-1 rounded p-2 w-24 ' value={inputs.pricePerNight} onChange={e => setInputs({ ...inputs, pricePerNight: e.target.value })} />
        </div>
      </div>
      ...
      <button className='bg-gray-800 hover:bg-gray-900 text-white px-8 py-2 rounded mt-8 cursor-pointer'>Add Room</button>
